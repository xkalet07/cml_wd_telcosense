...
epochs = 20
resume_epoch = 0 
learning_rate = 0.0002      # around 1e-4
kernel_size = 3             
...
## ---------------------------------------- CNN TRAINING ----------------------------------------
# train CNN model, save parameters, evaluate CNN autput for validation set 
...
# separating validation data which is the last 20 % 
cml = cml[int(len(cml)*0.8):]

cnn_out = cnn_out[int(len(cnn_out)*0.8):]
ref_wd = ref_wd[int(len(ref_wd)*0.8):]

# plot CNN classification output
